    ...
    for url in ctg[1:]:
        ...
        if requests.get(url_m).status_code == 200:
            while requests.get(url_m).status_code == 200:
                ...
                categ = soup_1.find("li", class_="active").text
                fichier = categ + ".csv"

                for lien in soup_1.find_all("h3"):
                    liens = lien.a["href"]
                    links.append(liens.replace("../../../", URL_CTL))
                    links_cat.append(liens.replace("../../../", URL_CTL))

                    with open(fichier, 'w') as AB:
                        writer = csv.DictWriter(AB, "")
                        writer.writerows(links_cat)
                    links_cat = []
                i = i + 1
                url_m = url.replace("index.html", "page-" + str(i) + ".html")
            ...
            categ = soup_2.find("li", class_="active").text
            fichier = categ + ".csv"
